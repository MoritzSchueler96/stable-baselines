...
def cnn_mlp_extractor(
    obs, net_arch, act_fun, obs_module_indices=None, dual_critic=False, **kwargs
):
    ...
    if shared or shared is None:
        ...
        mlps = mlp_extractor(
            conv_layer, net_arch, act_fun, obs_module_indices, dual_critic=dual_critic
        )
        if dual_critic:
        ...
        if not conv_vf:
            if len(obs.shape) == 3:
                vf_obs = vf_obs[:, 0, :]
            vf_mlps = mlp_extractor(
                vf_obs, [dict(vf=net_arch[-1]["vf"])], act_fun, dual_critic=dual_critic
            )
            if dual_critic:
            ...
            pi_mlp = mlp_extractor(
                pi_conv, [dict(pi=net_arch[-1]["pi"])], act_fun, dual_critic=False
            )[0]
        else:
            pi_conv = cnn_1d_extractor(pi_obs, act_fun, "pi_c1", **kwargs)
            ...
            vf_mlps = mlp_extractor(
                vf_conv, [dict(vf=net_arch[-1]["vf"])], act_fun, dual_critic=dual_critic
            )
            if dual_critic:
                _, vf_mlp, vf_mlp_d = vf_mlps
            else:
...
def mlp_extractor(
    flat_observations, net_arch, act_fun, obs_module_indices=None, dual_critic=False
):
    """
    ...
    for idx, (pi_layer_size, vf_layer_size) in enumerate(
        ...
        if vf_layer_size is not None:
            ...
            latent_value = act_fun(
                ...
                name="vf_fc{}_tanh".format(idx),
            )
            if dual_critic:
                latent_value_d = act_fun(
                    linear(
                        latent_value_d,
                        "vf_d_fc{}".format(idx),
                        vf_layer_size,
...
class ActorCriticPolicy(BasePolicy):
    ...
    def _setup_init(self):
        ...
        with tf.variable_scope("output", reuse=True):
            ...
            if self.dual_critic:
                self._value_flat = tf.minimum(
                    self.value_fn[:, 0], self.value_fn_d[:, 0]
                )
                self._critic_discrepancy = tf.square(
                    self.value_fn[:, 0] - self.value_fn_d[:, 0]
                )
            else:
...
class CnnMlpPolicy(FeedForwardPolicy):
    def __init__(
        ...
        super(CnnMlpPolicy, self).__init__(
            sess,
            ob_space,
            ac_space,
            n_env,
